...
#         observations = generation.shift(RIGHT).is_not_blank()
...
#             HDim(period, 'Period', CLOSEST, LEFT),
    ...
    #
        ...
        generators1 = tab.excel_ref('A5').expand(DOWN).is_not_blank()|total_electricity 
        generators = generators1 - within_which - share_totalGen - footnote
        fuel = tab.excel_ref('B5').expand(DOWN).is_not_blank() - share_totalGen - footnote 
        region = tab.excel_ref('C4').expand(RIGHT).is_not_blank()
#         period = region.shift(UP) 
        period = tab.excel_ref('C3').expand(RIGHT).is_not_blank()
...
#             if cell.value in ['TOTAL MPPs', 'TOTAL OTHER GENERATORS', 'TOTAL ALL GENERATING COMPANIES']:
...
#         df['Fuel Type'] = df['Fuel Type'].str.replace(r'\(.*\)', ' ')
        ...
        for column in tidy:
            if column in ('Generating Company', 'Fuel Type'):
                tidy[column] = tidy[column].map(lambda x: pathify(x))
