...
def process_big_table(anchor, task, trace):
    ...
    fuel_attributes = anchor.fill(DOWN).filter("Not fuel poor").assert_one().expand(RIGHT)
    obs = fuel_attributes.fill(DOWN)
    obs = clean_lower_tables(obs)
...
table_joins = {
    "Fuel poverty detailed tables - Energy Efficiency and Dwelling Characteristics - Aggregate Gap": {
        "category": "Aggregate fuel poverty Gap",
        "tables": "3through17",
        "comment": comment,
...
for title, info in table_joins.items():
    ...
    #cubes.add_cube(scraper, title, df)
    #cubes
